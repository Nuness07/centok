...
export default function PortfolioPage() {
  ...
  return (
    <main className="mx-auto max-w-[1280px] space-y-6 p-4 md:p-6">
      <div>
        <p className="text-xs font-black uppercase tracking-[0.18em] text-[#6F7A8F]">Account</p>
        <h1 className="mt-2 text-3xl font-black text-[#111827]">Portfolio</h1>
        <p className="mt-1 text-sm text-[#6F7A8F]">Holdings and available balance.</p>
      </div>
      {portfolio.isLoading ? <LoadingState label="Loading portfolio" /> : null}
      {portfolio.isError ? <ErrorState title="Portfolio unavailable" description="Reload or reset your workspace." onRetry={() => portfolio.refetch()} /> : null}
